...
public class StateAlg16  implements Comparable<StateAlg16> {
    ...
    public StateAlg16  (String [][]j , int level){
        int a= j.length;
    ...
    private  int manhattanDistance(){
        int sum=0;
        int [] index = new int [2];
        int a= Solution16Puzzle.goal1.length;
        for (int i =0;i<a;i++){
            for(int j=0; j<a;j++){
    ...
    private int [] findIndex(int element){
        ...
        for (int i =0 ; i<a;i++){
            for(int j=0;j<a;j++){
                if (Solution16Puzzle.goal1[i][j].trim().isEmpty()) {
                    continue;
                }
                if (Solution16Puzzle.goal1[i][j].trim().equals(String.valueOf(element))) {
                    index[0] = i;
                    index[1] = j;
                    return index;
                }
    ...
    public  ArrayList<StateAlg16> computeSuccessor (StateAlg16 main){
        ArrayList<StateAlg16>successor = new ArrayList<StateAlg16>();
            ...
            for(int j =0;j<a ; j++){
                if(main.Tiles[i][j].trim().isEmpty()){
                    ...
                    if(i-1>=0){
                        ...
                        StateAlg16 next = new StateAlg16( arr, main.level+1);
                        next.move="UP";
    ...
    @Override
    public int compareTo(StateAlg16 stateAlg) {
